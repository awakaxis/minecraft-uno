...
public class CardDeckRenderer implements BlockEntityRenderer<CardDeckBlockEntity> {
    ...
    @Override
    public void render(CardDeckBlockEntity blockEntity, float f, PoseStack poseStack,
            MultiBufferSource multiBufferSource, int i, int j) {
        if (blockEntity.getCardCount() <= 0)
            return;
        ...
        // TODO someday: translate with blockstate (i wanna make it so you can place the
        // deck on the left or right side of the block face, rather than just the
        // middle)
        ...
        // TODO: replace hardcoded vertex coordinates with variables so that
        // implementing blockstates is easier
        ...
        // round to x, x + 0.5, or x + 1 because the card side texture is actually two
        // cards on top of each other, so the value needs to be in terms of 0.5
    ...
    public void vertex(Matrix4f matrix4f, Matrix3f matrix3f, VertexConsumer vertexConsumer, float i, float j, float k,
